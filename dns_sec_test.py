...
def verify_hash(next_ns, name, curr_ns):
    ...
    ds_r = dns.query.udp(ds_q, curr_ns, timeout=1)
    k_q = dns.message.make_query(name, 48, want_dnssec=True)
    k_r = dns.query.udp(k_q, next_ns, timeout=1)
    '''
    (3: DSA/SHA-1, 5: RSA/SHA1, 8: 
RSA/SHA-256, 12: ECC-GOST
    '''
    if len(k_r.answer) == 0:
    ...
    for item in keys:
        if item.flags == 257: # KSK : 256, ZSK : 257
                            #
            KSK = item
    if KSK == None:
        print("No KSK")
        return 0
    ds_items = list(ds_r.answer[0].items.keys())
    alg=0
    if len(ds_items)==0:
        print("No DS record")
    else:
    ...
    for ns in nameservers:
        ...
        try: 
            ...
            if kname is not None:
                ...
                rrec = dns.query.udp(qrec, ns, timeout=1)
                qsec = dns.message.make_query(jname, 48, want_dnssec=True)
                rsec = dns.query.udp(qsec, ns, timeout=1)
                zsk = None
                print(rrec)
                if len(rsec.answer) > 0:
                    ...
                    try:
                        ## Verify the DNS Keys ##
                        dns.dnssec._validate(rsec.answer[0], rsec.answer[1], {jname:zsk})
                    except:
